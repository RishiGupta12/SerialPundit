...
1. Running this application
   ...
   application. Minicom/tertarem will show "test" as data received from this java program.
   Now type 10 or more characters in minicom/teraterm and they will appear in this Java
   program's console. 
...
2. What this application does and how it does
   ...
   It sends 4 bytes ("test") to serial device (host --> device) and then read data from serial 
   port, buffering until serial device has sent 10 or more bytes (host <-- device). When 10 
   or more bytes have been received it will print on console.
...
3. Going further
   ...
   - The purpose of this program is to give a simple example of getting started and do some 
     basic communication with serial device. See other examples for developing full applications.
