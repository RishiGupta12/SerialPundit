...
class DataPoller extends SerialTerminalMain implements Runnable {
	...
	public DataPoller(SerialComManager scm, JTextField text, long handle, JTextField status, Thread mDataReceiverThread) {
		...
		this.text = text;
		this.status = status;
		this.mDataReceiverThread = mDataReceiverThread;
	}
...
public class SerialTerminalMain extends JFrame {
	...
	private char xon;
	private char xoff;
	protected Thread mDataReceiverThread = null;
		...
		openButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					comPortHandle = scm.openComPort("/dev/pts/1", true, true, true);
					...
					mDataReceiverThread = new Thread(new DataPoller(scm, datarcvtextfield, comPortHandle, 
							programStatusText, mDataReceiverThread), "SCM DataPoller");
