 ...
 */
final class DataCommunicationHandler implements Runnable {
    ...
    private byte[] data;
    private byte[] dataBuffer = new byte[40];
    private int index = 0;
    ...
    public DataCommunicationHandler(SerialComManager scm, AtomicBoolean exitDataThread, Object dataLock, 
            AtomicBoolean isDevConnected, PortHandle phandle) {
        this.scm = scm;
    ...
    public void run() {
        ...
        while(exitDataThread.get() == false) {
            try {
                ...
                // Till the time serial device is connected in system, execute our application logic.
                q = 0; // reset.
                while(isDevConnected.get() == true) {
                    ...
                    try {
                        // let firmware prepare response.
                        Thread.sleep(50);
                    } catch(Exception e2) {
                    }

                    // Let us assume that the device will send response which is 15 bytes in size.
                    // So, keep buffering data until 15 or more than 15 bytes are received.
                    index = 0;
                    totalNumberOfBytesReadTillNow = 0;
                    while (totalNumberOfBytesReadTillNow < 15) {
                        try {
                            data = scm.readBytes(comPortHandle);
                            if(data != null) {
                            ...
                            System.out.println("Read status : " + e3.getMessage());
                            ...
                            // If the serial device get disconnected this loop will keep executing forever,
                            // so add a check before looping again.
                ...
                // update appropriate variables. This handles the case also where user plugs in device 
...
final class ConcreteUSBHotPlugEventHandler implements Runnable, ISerialComUSBHotPlugListener {
    ...
    private final int PRODUCT_VID = 0x0403;
    ...
    private static RingArrayBlockingQueue<Integer> mUSBhotplugEventQueue = new RingArrayBlockingQueue<Integer>(1024);
    ...
    public void run() {
        ...
        dch = new DataCommunicationHandler(scm, exitDataThread, dataLock, isDevConnected, phandle);
            ...
            try {
                ...
                if(hotplugEvent == SerialComUSB.DEV_ADDED) {
                    if((eventSerialize == 1) && (comPortHandle == -1)) {
                        ...
                        // and given correct permissions. This delay can be tuned as per the 
                        // operating system.
                        Thread.sleep(1500);
...
/** 
 * This class initializes variables and setup required for operation.
 */
public final class HotPlugRecoveryApplication {
    ...
    public static void main(String[] args) {
        try {			
            ...
            // On application exit un-register usb hot plug listener.
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
