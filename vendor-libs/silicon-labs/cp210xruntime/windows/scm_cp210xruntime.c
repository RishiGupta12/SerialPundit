...
*/
JNIEXPORT jint JNICALL Java_com_embeddedunveiled_serial_internal_SerialComCP210xRuntimeJNIBridge_readLatch(JNIEnv *env, 
...
* @throws SerialComException if any CP210xRT function, JNI function, system call or C function fails.
*/
JNIEXPORT jint JNICALL Java_com_embeddedunveiled_serial_internal_SerialComCP210xRuntimeJNIBridge_writeLatch(JNIEnv *env, 
	jobject obj, jlong handle, jlong maskValue, jlong latchValue) {
...
* @throws SerialComException if any CP210xRT function, JNI function, system call or C function fails.
*/
JNIEXPORT jstring JNICALL Java_com_embeddedunveiled_serial_internal_SerialComCP210xRuntimeJNIBridge_getPartNumber(JNIEnv *env, 
	jobject obj, jlong handle) {
	...
	_snprintf_s(number, 16, 16, "CP21%02d", part_num);
...
* @throws SerialComException if any CP210xRT function, JNI function, system call or C function fails.
*/
JNIEXPORT jstring JNICALL Java_com_embeddedunveiled_serial_internal_SerialComCP210xRuntimeJNIBridge_getDeviceProductString(JNIEnv *env, 
	jobject obj, jlong handle) {
	...
	memset(product, '\0', sizeof(CP210x_PRODUCT_STRING));
	ret = CP210xRT_GetDeviceProductString((HANDLE)handle, product, &length, TRUE);
	if (ret != CP210x_SUCCESS) {
		throw_serialcom_exception(env, 2, ret, NULL);
		return NULL;
	}
...
* @throws SerialComException if any CP210xRT function, JNI function, system call or C function fails.
*/
JNIEXPORT jstring JNICALL Java_com_embeddedunveiled_serial_internal_SerialComCP210xRuntimeJNIBridge_getDeviceSerialNumber(JNIEnv *env, 
	jobject obj, jlong handle) {
	...
	memset(serial_number, '\0', sizeof(CP210x_SERIAL_STRING));
	ret = CP210xRT_GetDeviceSerialNumber((HANDLE)handle, serial_number, &length, TRUE);
	if (ret != CP210x_SUCCESS) {
		throw_serialcom_exception(env, 2, ret, NULL);
		return NULL;
	}
...
* @throws SerialComException if any CP210xRT function, JNI function, system call or C function fails.
*/
JNIEXPORT jstring JNICALL Java_com_embeddedunveiled_serial_internal_SerialComCP210xRuntimeJNIBridge_getDeviceInterfaceString(JNIEnv *env, 
	jobject obj, jlong handle) {
	...
	ret = CP210xRT_GetDeviceInterfaceString((HANDLE)handle, iface, &length, TRUE);
	if (ret != CP210x_SUCCESS) {
		throw_serialcom_exception(env, 2, ret, NULL);
		return NULL;
	}

	info = (*env)->NewStringUTF(env, iface);
	if ((info == NULL) || ((*env)->ExceptionOccurred(env) != NULL)) {
		throw_serialcom_exception(env, 3, 0, E_NEWSTRUTFSTR);
		return NULL;
	}
