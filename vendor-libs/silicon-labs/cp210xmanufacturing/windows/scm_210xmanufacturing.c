...
(JNIEnv *env, jobject obj, jint index, jint flag) {
	...
	memset(dev_buffer, '\0', sizeof(CP210x_DEVICE_STRING));
...
(JNIEnv *env, jobject obj, jlong handle) {
	...
	memset(manufacturer, '\0', sizeof(CP210x_MANUFACTURER_STRING));
...
(JNIEnv *env, jobject obj, jlong handle) {
	...
	memset(serial, '\0', sizeof(CP210x_SERIAL_STRING));
...
(JNIEnv *env, jobject obj, jlong handle, jintArray resetLatch, jintArray suspendLatch, jbyteArray config) {
	...
	CP210x_STATUS ret = 0;
	QUAD_PORT_CONFIG port_config = { 0 };
