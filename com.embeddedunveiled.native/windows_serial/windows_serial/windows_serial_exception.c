...
void throw_serialcom_exception(JNIEnv *env, int type, int error_code, const char *msg) {
	...
	case 2:
		...
		memset(buffer, '\0', sizeof(buffer));
		switch (error_code) {
		case E_CALLOC: strcpy_s(buffer, sizeof(buffer), E_CALLOCSTR);
			break;
		case E_ATTACHCURRENTTHREAD: strcpy_s(buffer, sizeof(buffer), E_ATTACHCURRENTTHREADSTR);
			break;
		case E_GETOBJECTCLASS: strcpy_s(buffer, sizeof(buffer), E_GETOBJECTCLASSSTR);
			break;
		case E_GETMETHODID: strcpy_s(buffer, sizeof(buffer), E_GETMETHODIDSTR);
			break;
		case E_CALLVOIDMETHD: strcpy_s(buffer, sizeof(buffer), E_CALLVOIDMETHDSTR);
			break;
		default: strcpy_s(buffer, sizeof(buffer), E_UNKNOWN);
		}
		ret = (*env)->ThrowNew(env, serialComExceptionClass, buffer);
		if (ret < 0) {
			LOGE(FAILTHOWEXP, buffer);
