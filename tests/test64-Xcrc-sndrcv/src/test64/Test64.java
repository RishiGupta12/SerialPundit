...
class SendText extends Test64 implements Runnable {
	@Override
	public void run() {
		try {
...
class SendBinary extends Test64 implements Runnable {
	@Override
	public void run() {
		try {
			long handle1 = scm.openComPort(PORT1, true, true, true);
...
public class Test64 {
	...
	public static void main(String[] args) {
		try {
			...
			mThread.start();
			scm.receiveFile(handle, new File(rcvtfilepath), FTPPROTO.XMODEM, FTPVAR.CRC, true);
			...
			mThread.start();
			scm.receiveFile(handle, new File(rcvbfilepath), FTPPROTO.XMODEM, FTPVAR.CRC, false);
