...
public final class SerialComBluetoothJNIBridge {
	 ...
	 * 
	 * @param directoryPath
	 *            null for default directory or user supplied directory path.
	 * @param loadedLibName
	 *            null for default name or user supplied name of loaded library.
	 * @param serialComSystemProperty
	 *            instance of SerialComSystemProperty to get required java
	 *            properties.
	 * @param cpuArch
	 *            architecture of CPU this library is running on.
	 * @param osType
	 *            operating system this library is running on.
	 * @param javaABIType
	 *            binary application interface type to correctly link.
	 * @throws SecurityException
	 *             if java system properties can not be accessed.
	 * @throws SerialComUnexpectedException
	 *             if java system property is null.
	 * @throws SerialComLoadException
	 *             if any file system related issue occurs.
	...
	public static boolean loadNativeLibrary(String directoryPath,
		...
		/*
		 * Prepare directory in which native shared library will be extracted
		 * from jar
		 */
		if (directoryPath == null) {
			...
			if (baseDir.exists() && baseDir.isDirectory() && baseDir.canWrite()) {
				...
				// access to temp directory failed, let us try access to user's
				// home directory
				userHomeDir = serialComSystemProperty.getUserHome();
				if (userHomeDir == null) {
					throw new SerialComUnexpectedException(
							"The user.home java system property is null in the system !");
				}
				baseDir = new File(userHomeDir);
				if (!baseDir.exists()) {
					throw new SerialComLoadException(
							"User home directory does not exist. Also unable to access tmp/temp directory !");
				}
				if (!baseDir.isDirectory()) {
					throw new SerialComLoadException(
							"User home directory is not a directory. Also unable to access tmp/temp directory !");
				}
				if (!baseDir.canWrite()) {
					throw new SerialComLoadException(
							"User home directory is not writeable (permissions ??). Also unable to access tmp/temp directory !");
				}
				isUserHomeDir = true;
			}

			// for tmp or user home create unique directory inside them for our
			// use only
			workingDir = new File(baseDir.toString() + fileSeparator
					+ "scm_tuartx1");
			if (!workingDir.exists()) {
				if (!workingDir.mkdir()) {
					if (isTmpDir == true) {
						throw new SerialComLoadException(
								"Can not create scm_tuartx1 unique directory in temp directory !");
					} else if (isUserHomeDir == true) {
						throw new SerialComLoadException(
								"Can not create scm_tuartx1 unique directory in user home directory !");
					} else {
					}
				}
			}
			...
			if (!baseDir.exists()) {
				throw new SerialComLoadException("Given " + directoryPath
			...
			// for user specified directory base itself will be working
			// directory
			workingDir = baseDir;
		}

		/*
		 * Find the native library that will be extracted based on arch and os
		 * type
		 */
		if (cpuArch == SerialComManager.ARCH_AMD64) {
			switch (osType) {
			case SerialComManager.OS_WINDOWS:
				libToExtractFromJar = "win" + stackName + "_"
						+ SerialComManager.JAVA_LIB_VERSION + "_x86_64.dll";
				libExtension = ".dll";
				break;
			case SerialComManager.OS_LINUX:
				libToExtractFromJar = "linux" + stackName + "_"
						+ SerialComManager.JAVA_LIB_VERSION + "_x86_64.so";
				libExtension = ".so";
				break;
			case SerialComManager.OS_MAC_OS_X:
				libToExtractFromJar = "mac" + stackName + "_"
						+ SerialComManager.JAVA_LIB_VERSION + "_x86_64.dylib";
				libExtension = ".dylib";
				break;
			default:
			}
			...
			case SerialComManager.OS_WINDOWS:
				libToExtractFromJar = "win" + stackName + "_"
						+ SerialComManager.JAVA_LIB_VERSION + "_x86.dll";
				libExtension = ".dll";
				break;
			case SerialComManager.OS_LINUX:
				libToExtractFromJar = "linux" + stackName + "_"
						+ SerialComManager.JAVA_LIB_VERSION + "_x86.so";
				libExtension = ".so";
				break;
			case SerialComManager.OS_MAC_OS_X:
				libToExtractFromJar = "mac" + stackName + "_"
						+ SerialComManager.JAVA_LIB_VERSION + "_x86.dylib";
				libExtension = ".dylib";
				break;
			default:
			}
			...
			if (osType == SerialComManager.OS_LINUX) {
				...
				if (javaABIType == SerialComManager.ABI_ARMHF) {
					libToExtractFromJar = "linux" + stackName + "_"
							+ SerialComManager.JAVA_LIB_VERSION + "_armv7hf.so";
				} else if (javaABIType == SerialComManager.ABI_ARMEL) {
					libToExtractFromJar = "linux" + stackName + "_"
							+ SerialComManager.JAVA_LIB_VERSION + "_armv7el.so";
				} else {
				}
			}
			...
			if (osType == SerialComManager.OS_LINUX) {
				...
				if (javaABIType == SerialComManager.ABI_ARMHF) {
					libToExtractFromJar = "linux" + stackName + "_"
							+ SerialComManager.JAVA_LIB_VERSION + "_armv6hf.so";
				} else if (javaABIType == SerialComManager.ABI_ARMEL) {
					libToExtractFromJar = "linux" + stackName + "_"
							+ SerialComManager.JAVA_LIB_VERSION + "_armv6el.so";
				} else {
				}
			}
			...
			if (osType == SerialComManager.OS_LINUX) {
				libExtension = ".so";
				libToExtractFromJar = "linux" + stackName + "_"
						+ SerialComManager.JAVA_LIB_VERSION + "_armv5.so";
			}
		} else {
		}
		...
		try {
			if (loadedLibName == null) {
				libFile = new File(workingDir.getAbsolutePath() + fileSeparator
						+ libToExtractFromJar);
			} else {
				libFile = new File(workingDir.getAbsolutePath() + fileSeparator
			...
			if (input != null) {
				...
				if ((libFile != null) && libFile.exists() && libFile.isFile()) {
					...
					throw new SerialComLoadException(
							"Can not extract native shared library "
									+ libToExtractFromJar
									+ " from scm-x.x.x.jar file !");
				}
			} else {
				throw new SerialComLoadException(
						"Can not get shared library "
								+ libToExtractFromJar
								+ " resource as stream from scm-x.x.x.jar file using class loader !");
			}
		} catch (Exception e) {
			throw (SerialComLoadException) new SerialComLoadException(
					libFile.toString()).initCause(e);
		} finally {
			try {
				if (output != null) {
					output.close();
		...
		try {
			...
			throw (UnsatisfiedLinkError) new UnsatisfiedLinkError(
					"Could not load " + libFile.toString()
							+ " native library !").initCause(e);
		}
