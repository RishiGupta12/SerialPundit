 ...
 * in 'lib-mapper' folder in 'scm-x.x.x.jar' file.</p>
 * 
 * @author Rishi Gupta
 */
public final class SerialComPortMapperJNIBridge {
	...
	/**
	 * <p>Allocates a new SerialComPortMapperJNIBridge object.</p>
	 */
	public SerialComPortMapperJNIBridge() {
	}

	/**
	 * <p>Extract native library from jar in a working directory, load and link it. The 'lib-mapper' folder in 
	 * 'scm-x.x.x.jar'.</p> 
	 * 
	...
	public static boolean loadNativeLibrary(String directoryPath, String loadedLibName, SerialComSystemProperty serialComSystemProperty,
		...
		if(cpuArch == SerialComManager.ARCH_AMD64) {
			...
			case SerialComManager.OS_WINDOWS:
				libToExtractFromJar = "winmap_" + SerialComManager.JAVA_LIB_VERSION + "_x86_64.dll";
				libExtension = ".dll";
				break;
			case SerialComManager.OS_LINUX:
				libToExtractFromJar = "linuxmap_" + SerialComManager.JAVA_LIB_VERSION + "_x86_64.so";
				libExtension = ".so";
				break;
			case SerialComManager.OS_MAC_OS_X:
				libToExtractFromJar = "macmap_" + SerialComManager.JAVA_LIB_VERSION + "_x86_64.dylib";
				libExtension = ".dylib";
				break;
			default :
			}
			...
			switch(osType) {
			case SerialComManager.OS_WINDOWS:
				libToExtractFromJar = "winmap_" + SerialComManager.JAVA_LIB_VERSION + "_x86.dll";
				libExtension = ".dll";
				break;
			case SerialComManager.OS_LINUX:
				libToExtractFromJar = "linuxmap_" + SerialComManager.JAVA_LIB_VERSION + "_x86.so";
				libExtension = ".so";
				break;
			case SerialComManager.OS_MAC_OS_X:
				libToExtractFromJar = "macmap_" + SerialComManager.JAVA_LIB_VERSION + "_x86.dylib";
				libExtension = ".dylib";
				break;
			default :
			}
			...
			if(osType == SerialComManager.OS_LINUX) {
				...
				if(javaABIType == SerialComManager.ABI_ARMHF) {
					libToExtractFromJar = "linuxmap_" + SerialComManager.JAVA_LIB_VERSION + "_armv7hf.so";
				}else if(javaABIType == SerialComManager.ABI_ARMEL) {
					libToExtractFromJar = "linuxmap_" + SerialComManager.JAVA_LIB_VERSION + "_armv7el.so";
				}else {
				}
			}
			...
			if(osType == SerialComManager.OS_LINUX) {
				libExtension = ".so";
				if(javaABIType == SerialComManager.ABI_ARMHF) {
					libToExtractFromJar = "linuxmap_" + SerialComManager.JAVA_LIB_VERSION + "_armv6hf.so";
				}else if(javaABIType == SerialComManager.ABI_ARMEL) {
					libToExtractFromJar = "linuxmap_" + SerialComManager.JAVA_LIB_VERSION + "_armv6el.so";
				}else {
				}
			}
			...
			if(osType == SerialComManager.OS_LINUX) {
				libExtension = ".so";
				libToExtractFromJar = "linuxmap_" + SerialComManager.JAVA_LIB_VERSION + "_armv5.so";
			}
		}else {
		}
		...
		try {
			...
			input = SerialComPortMapperJNIBridge.class.getResourceAsStream("/lib-mapper/" + libToExtractFromJar);
	...
	public native int mapAliasToExistingComPort(String alias, String comPort);
	public native int unmapAliasToExistingComPort(String alias);
}
